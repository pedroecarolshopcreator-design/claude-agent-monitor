  ...
  return (
    ...
    >
      <div className="flex items-center gap-1">
        <span className={`${statusColor} shrink-0 ${agent.status === 'active' ? 'terminal-glow' : ''}`}>
          {statusChar}
        </span>
        <span className={`truncate ${isSelected ? 'text-[#00ff00] terminal-glow' : 'text-[#00cc00]'}`}>
          {agent.name}
        </span>
      </div>
