    ...
    <div className="h-full flex flex-col font-mono text-[11px]">
      ...
      <div className="p-2 border-b border-[#1a3a1a] shrink-0">
        ...
        <div className="terminal-dim">
          <span className={agent.status === 'active' ? 'text-[#00ff00]' : agent.status === 'error' ? 'terminal-error' : 'terminal-muted'}>
            {statusChar}
          </span>
        ...
        <div className="mt-2 border border-[#1a3a1a] p-1.5">
          <div className="text-[10px] terminal-dim mb-1">{'\u250C\u2500\u2500 STATS \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2510'}</div>
          <div className="grid grid-cols-3 gap-x-2 text-[10px]">
            ...
            <div>
              ...
              <span className={`ml-1 ${agent.errorCount > 0 ? 'terminal-error' : 'text-[#00ff00]'}`}>
                {agent.errorCount}
              </span>
            </div>
            <div>
          ...
          <div className="text-[10px] terminal-dim mt-1">{'\u2514\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2518'}</div>
        </div>
      </div>
        ...
        {activeTab === 'activity' && (
          <div>
            {agentEvents.length === 0 ? (
              <p className="terminal-dim p-3 text-center">{'> No activity recorded'}</p>
            ) : (
              agentEvents.slice(0, 100).map((event) => (
                <div key={event.id} className="px-2 py-0.5 hover:bg-[#0a1a0a] transition-colors border-b border-[#0a1a0a]">
                  <div className="flex items-center gap-2">
                    <span className="terminal-dim shrink-0 text-[10px]">
                      {formatTimestamp(event.timestamp)}
                    </span>
                    <span className={`text-[10px] ${event.category === 'error' ? 'terminal-error' : 'text-[#00ccff]'}`}>
                      {event.tool || event.hookType}
                    </span>
                    {event.filePath && (
                      <span className="text-[#00aa00] truncate text-[10px]">
                        {truncatePath(event.filePath, 25)}
                      </span>
                    )}
                  </div>
                </div>
          ...
          <div className="p-2">
            {agentFiles.length === 0 ? (
              ...
              agentFiles.map((filePath, i) => {
                ...
                const branch = isLast ? '\u2514\u2500' : '\u251C\u2500';
                return (
                  <div key={filePath} className="flex items-center gap-1 py-0.5 hover:bg-[#0a1a0a]">
        ...
        {activeTab === 'errors' && (
          <div>
            {agentErrors.length === 0 ? (
              <p className="terminal-dim p-3 text-center">{'> No errors - all clear'}</p>
            ) : (
              agentErrors.map((event) => (
                <div key={event.id} className="px-2 py-1 border-b border-[#1a0a0a]">
                  <div className="flex items-center gap-2">
          ...
          <div>
            {agentMessages.length === 0 ? (
              ...
              agentMessages.map((event) => (
                <div key={event.id} className="px-2 py-1 border-b border-[#0a1a0a]">
                  <div className="flex items-center gap-2">
                    <span className="terminal-dim text-[10px]">
                      {formatTimestamp(event.timestamp)}
                    </span>
                    <span className="terminal-info text-[10px]">SendMessage</span>
                  </div>
                  {event.input && (
                    <p className="text-[#00aa00] text-[10px] ml-2 mt-0.5">
