      ...
      {files.length === 0 ? (
        ...
        <div className="flex-1 overflow-y-auto terminal-scrollbar space-y-0">
          {files.map((file, i) => {
            ...
            const ch = CHANGE_CHARS[file.changeType] || { symbol: '?', color: 'terminal-dim' };
            const branch = isLast ? '\u2514\u2500\u2500' : '\u251C\u2500\u2500';
            ...
            return (
              ...
              >
                ...
                <span className={`shrink-0 w-[12px] font-bold ${ch.color}`}>{ch.symbol}</span>
                <span className="text-[#00aa00] truncate flex-1 ml-1">
