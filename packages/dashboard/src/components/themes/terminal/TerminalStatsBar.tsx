...
export function TerminalStatsBar() {
  ...
  const stats = [
    ...
    { label: 'AGENTS', value: `${activeAgents}/${agents.length}`, isError: false },
    { label: 'CALLS', value: formatNumber(totalToolCalls), isError: false },
    { label: 'EVENTS', value: formatNumber(events.length), isError: false },
      ...
      <div className="flex items-center gap-0">
        ...
        {stats.map((stat, i) => (
          <span key={stat.label} className="flex items-center">
            ...
            <span className={`ml-1 font-bold ${stat.isError ? 'terminal-error' : 'text-[#00ff00] terminal-glow'}`}>
              {stat.value}
            </span>
          </span>
        ))}
