...
function getCategoryIcon(category: string): string {
  switch (category) {
    ...
    case 'file_change': return '\u{1F4E6}';
    case 'command': return '\u{1F4BB}';
    case 'message': return '\u{1F4AC}';
    case 'lifecycle': return '\u{1F504}';
    case 'error': return '\u2620';
    case 'compact': return '\u{1F4E6}';
  ...
  return (
    <div className="h-full flex flex-col" style={{ background: 'var(--pixel-bg)' }}>
      ...
      <div
        ...
        style={{ borderBottom: '3px solid var(--pixel-border)', background: 'var(--pixel-surface)' }}
      >
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto pixel-scrollbar">
        {filteredEvents.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center p-8">
            ...
            <p className="pixel-text-xs mt-2 pixel-blink" style={{ color: 'var(--pixel-gold)' }}>
              AWAITING BATTLE...
            </p>
          </div>
        ) : (
          <div>
            {filteredEvents.map((event, index) => (
              <div key={event.id} className="pixel-slide-in" style={{ animationDelay: `${Math.min(index * 0.02, 0.2)}s` }}>
                <EventItem
                  event={event}
...
function EventItem({
  ...
  const action = TOOL_ACTIONS[event.tool || ''] || `used ${(event.tool || event.hookType).toUpperCase()}!`;
  ...
  return (
    <div
      ...
      onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.background = 'var(--pixel-surface)'; }}
      onMouseLeave={(e) => {
    ...
    >
      <div className="flex items-start gap-3">
        {/* Icon */}
        <span className="pixel-text-base mt-0.5 w-5 text-center shrink-0">{icon}</span>
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* Effectiveness */}
          <span className="pixel-text-xs" style={{ color: effectiveness.color }}>
            {effectiveness.text}
          </span>
          ...
          {event.filePath && (
            <div className="mt-0.5">
              <span className="pixel-text-xs" style={{ color: 'var(--pixel-text-dim)' }}>
                TARGET: {truncatePath(event.filePath)}
              </span>
            </div>
          )}
          ...
          {/* Error display */}
          {event.error && (
            <p className="pixel-text-xs mt-0.5 pixel-shake" style={{ color: 'var(--pixel-error)' }}>
              \u2620 {event.error}
            </p>
          )}
        ...
        {/* Timestamp */}
        <span className="pixel-text-xs shrink-0" style={{ color: 'var(--pixel-text-dim)' }}>
