...
export function PixelAgentDetail() {
  ...
  const agentErrors = agentEvents.filter((e) => e.category === 'error' || e.error);
  ...
  const xpMax = 50;
  const hp = agent.errorCount > 0 ? Math.max(10, 100 - agent.errorCount * 20) : 100;
  ...
  return (
    <div className="h-full flex flex-col" style={{ background: 'var(--pixel-bg)' }}>
      {/* Character Sheet Header */}
      <div className="p-3 shrink-0" style={{ borderBottom: '3px solid var(--pixel-border)', background: 'var(--pixel-surface)' }}>
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            >
              <span className="pixel-text-lg font-bold">{agent.name.charAt(0).toUpperCase()}</span>
            </div>
            <div>
              <h3 className="pixel-text-sm" style={{ color: 'var(--pixel-text)' }}>{agent.name}</h3>
        ...
        <div className="space-y-1 mb-2">
          <div className="flex items-center gap-1">
            <span className="pixel-text-xs" style={{ color: 'var(--pixel-hp)', width: '20px' }}>HP</span>
            <div className="flex-1 pixel-bar-container pixel-bar-hp">
              <div className="pixel-bar-fill" style={{ width: `${hp}%` }} />
            </div>
            <span className="pixel-text-xs" style={{ color: 'var(--pixel-text-muted)', width: '40px', textAlign: 'right' }}>
              {hp}/100
            </span>
          </div>
          <div className="flex items-center gap-1">
            <span className="pixel-text-xs" style={{ color: 'var(--pixel-mp)', width: '20px' }}>MP</span>
          ...
          <div className="flex items-center gap-1">
            <span className="pixel-text-xs" style={{ color: 'var(--pixel-xp)', width: '20px' }}>XP</span>
            <div className="flex-1 pixel-bar-container pixel-bar-xp">
              <div className="pixel-bar-fill" style={{ width: `${(xpCurrent / xpMax) * 100}%` }} />
            </div>
            <span className="pixel-text-xs" style={{ color: 'var(--pixel-text-muted)', width: '40px', textAlign: 'right' }}>
              {xpCurrent}/{xpMax}
            </span>
        ...
        <div className="grid grid-cols-4 gap-1">
          {[
            ...
            <div
              ...
              style={{ background: 'var(--pixel-bg-dark)', border: '2px solid var(--pixel-border)' }}
            >
              <div className="pixel-text-sm font-bold" style={{ color: stat.color }}>{stat.value}</div>
              <div className="pixel-text-xs" style={{ color: 'var(--pixel-text-dim)' }}>{stat.label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Tabs */}
      <div className="flex shrink-0" style={{ borderBottom: '3px solid var(--pixel-border)' }}>
        {TABS.map((tab) => {
          const count =
            tab.id === 'activity' ? agentEvents.length :
            tab.id === 'files' ? agentFiles.length :
            tab.id === 'errors' ? agentErrors.length :
          ...
          return (
            <button
              ...
              style={{
                background: activeTab === tab.id ? 'var(--pixel-accent)' : 'var(--pixel-surface)',
                color: activeTab === tab.id ? '#fff' : 'var(--pixel-text-muted)',
        ...
        {activeTab === 'activity' && (
          <div>
            {agentEvents.length === 0 ? (
              <p className="pixel-text-xs p-4 text-center" style={{ color: 'var(--pixel-text-muted)' }}>
              ...
              agentEvents.slice(0, 100).map((event) => (
                ...
                >
                  ...
                  {event.filePath && (
                    <span
                      className="pixel-text-xs block mt-0.5 truncate"
                      style={{ color: 'var(--pixel-text-muted)' }}
                    >
                      {truncatePath(event.filePath, 30)}
                    </span>
                  )}
                </div>
              ))
        ...
        {activeTab === 'errors' && (
          <div>
            {agentErrors.length === 0 ? (
              <p className="pixel-text-xs p-4 text-center" style={{ color: 'var(--pixel-green)' }}>
                NO CURSES! ALL CLEAR!
              </p>
            ) : (
              agentErrors.map((event) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="pixel-text-xs" style={{ color: 'var(--pixel-error)' }}>
                      \u2620 {event.tool || event.hookType}
                    </span>
                    <span className="pixel-text-xs" style={{ color: 'var(--pixel-text-dim)' }}>
                      {formatTimestamp(event.timestamp)}
                  ...
                  {event.error && (
                    <p className="pixel-text-xs mt-0.5 break-all" style={{ color: 'var(--pixel-error)' }}>
                      {event.error}
                    </p>
                  )}
                </div>
            ...
            {agentMessages.length === 0 ? (
              ...
              agentMessages.map((event) => (
                ...
                >
                  ...
                  {event.input && (
                    <p className="pixel-text-xs mt-0.5" style={{ color: 'var(--pixel-text-muted)' }}>
