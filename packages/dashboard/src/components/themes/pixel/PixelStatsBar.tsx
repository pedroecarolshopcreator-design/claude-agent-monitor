...
export function PixelStatsBar() {
  ...
  useEffect(() => {
    ...
    const update = () => setElapsed(formatElapsedTime(session.startedAt, session.endedAt));
    update();
  ...
  const stats = [
    { label: 'TIME', value: elapsed, icon: STAT_ICONS.TIME, color: 'var(--pixel-text)' },
    {
      label: 'PARTY',
    ...
    { label: 'SKILLS', value: formatNumber(totalToolCalls), icon: STAT_ICONS.SKILLS, color: 'var(--pixel-cyan)' },
    { label: 'EVENTS', value: formatNumber(events.length), icon: STAT_ICONS.EVENTS, color: 'var(--pixel-text)' },
    { label: 'LOOT', value: formatNumber(fileEvents), icon: STAT_ICONS.LOOT, color: 'var(--pixel-orange)' },
    {
      label: 'POISON',
    ...
    >
      {stats.map((stat, i) => (
        ...
        >
          ...
          <span className="pixel-text-sm" style={{ color: stat.color, fontWeight: 'bold' }}>
            {stat.value}
          </span>
        </div>
      ))}
